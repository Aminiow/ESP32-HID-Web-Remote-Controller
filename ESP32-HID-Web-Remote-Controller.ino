    ...
    <div class="container">
      <div class="card full-width">
        ...
        >
          ...
          <a
            href="/sta"
            style="color: #5b9aff; font-size: 20px; text-decoration: none"
            >📶</a
          >
        </div>
        <div class="sta-status" id="staStatus">
          ...
          <span style="margin-left: auto"
            ><button
              onclick="window.location.href = '/sta'"
              style="background: #333; padding: 4px 12px; font-size: 12px"
            >
              Settings
            </button></span
          >
      ...
      <div class="card">
        ...
        <div class="btn-group">
          <button class="accent" onclick="sendHTTP('/click?btn=left')">
            Left
          </button>
          <button class="accent" onclick="sendHTTP('/click?btn=right')">
            Right
          </button>
          <button class="accent" onclick="sendHTTP('/click?btn=middle')">
            Middle
          </button>
          <button class="accent" onclick="sendHTTP('/double?btn=left')">
            Double
          </button>
        </div>
        <div class="btn-group">
          <button id="mouseLeftDown" data-btn="left">L⬇</button>
          <button id="mouseLeftUp" data-btn="left" onclick="mouseUp('left')">
            L⬆
          </button>
          <button id="mouseRightDown" data-btn="right">R⬇</button>
          <button id="mouseRightUp" data-btn="right" onclick="mouseUp('right')">
            R⬆
          </button>
        ...
        <div class="small">
          Drag on pad to move. Tap for left click. Arrows hold-to-repeat.
        </div>
      </div>
      <div class="card">
        <h3>⌨ Keyboard</h3>
        ...
        <div class="small">
          Sticky modifiers toggled via buttons above. Keyboard keys
    ...
    <script>
      ...
      function sendMove(dx, dy) {
        ...
        if (socket && socket.readyState === WebSocket.OPEN) {
          ...
          sendHTTP(
            "/move?dx=" +
              encodeURIComponent(realDx) +
              "&dy=" +
              encodeURIComponent(realDy),
          );
        }
      }
      let sens = 2.0;
      ...
      document
        ...
        .addEventListener("input", function () {
          ...
          repeatSaveTimer = setTimeout(() => {
            sendHTTP(
              "/set_repeat?value=" + encodeURIComponent(repeatInterval),
            ).catch(() => {});
          }, 400);
          logInfo("Repeat interval = " + repeatInterval);
        });
      document
        .getElementById("legacyCheck")
      ...
      function updateMouseUI() {
        document
          .getElementById("mouseLeftDown")
          .classList.toggle("pressed", mouseState.left);
        document
          .getElementById("mouseRightDown")
          .classList.toggle("pressed", mouseState.right);
      }
      function mouseDown(btn) {
        if (btn === "left" && !mouseState.left) {
          mouseState.left = true;
          sendHTTP("/down?btn=left").catch(() => {});
        } else if (btn === "right" && !mouseState.right) {
          mouseState.right = true;
          sendHTTP("/down?btn=right").catch(() => {});
        }
        updateMouseUI();
      }
      function mouseUp(btn) {
        if (btn === "left" && mouseState.left) {
          ...
          sendHTTP("/up?btn=left").catch(() => {});
        } else if (btn === "right" && mouseState.right) {
          mouseState.right = false;
          sendHTTP("/up?btn=right").catch(() => {});
        }
        updateMouseUI();
      }
      ...
      function releaseKey(code) {
        const count = heldKeyCounts.get(code) || 0;
        if (count <= 1) heldKeyCounts.delete(code);
        else heldKeyCounts.set(code, count - 1);
      }
      function releaseHeldKeys() {
        ...
        heldKeyCounts.clear();
        document
          .querySelectorAll(".mod-down")
          .forEach((el) => el.classList.remove("mod-down"));
        sendHTTP("/reset_modifiers").catch(() => {});
        Object.keys(modState).forEach((k) => {
          modState[k] = false;
        });
        document.querySelectorAll("#modButtons button").forEach((btn) => {
          ...
          row.forEach(([label, size]) => {
            ...
            if (isMod) {
              ...
              el.addEventListener("click", () => {
                ...
                if (code) {
                  sendHTTP("/key?key=" + encodeURIComponent(code)).catch(
                    () => {},
                  );
                } else if (label.length === 1) {
                  sendHTTP("/type?text=" + encodeURIComponent(label)).catch(
                    () => {},
                  );
                } else {
                  sendHTTP("/key?key=" + encodeURIComponent(label)).catch(
                    () => {},
                  );
                }
                highlightKey(el);
      ...
      function buildNumpad() {
        ...
        plus.addEventListener("click", () => {
          sendHTTP("/key?key=KP_PLUS").catch(() => {});
          highlightKey(plus);
        });
        ...
        enter.addEventListener("click", () => {
          sendHTTP("/key?key=KP_ENTER").catch(() => {});
          highlightKey(enter);
        });
      ...
      realInput.addEventListener("focus", () => {
        realtimeOld = realInput.value;
      });
      realInput.addEventListener("input", function () {
        const newVal = this.value;
        if (
          newVal.startsWith(realtimeOld) &&
          newVal.length > realtimeOld.length
        ) {
        ...
        if (
          realtimeOld.startsWith(newVal) &&
          newVal.length < realtimeOld.length
        ) {
          const delCount = realtimeOld.length - newVal.length;
          for (let i = 0; i < delCount; i++)
            sendHTTP("/key?key=BACKSPACE").catch(() => {});
          realtimeOld = newVal;
          return;
        }
      ...
      async function testAll() {
        ...
        try {
          await sendHTTP("/click?btn=left");
          await sleep(200);
          await sendHTTP("/click?btn=right");
          await sleep(200);
          await sendHTTP("/move?dx=30&dy=0");
          await sleep(200);
          await sendHTTP("/move?dx=0&dy=30");
          await sleep(200);
          await sendHTTP("/wheel?delta=1");
          await sleep(200);
          await sendHTTP("/type?text=Hello");
          await sleep(300);
          await sendHTTP("/key?key=ENTER");
          await sleep(200);
          await sendHTTP("/key?key=BACKSPACE");
          await sleep(200);
          await sendHTTP("/toggle_modifier?mod=SHIFT");
          await sleep(200);
          await sendHTTP("/type?text=a");
          await sleep(200);
          await sendHTTP("/toggle_modifier?mod=SHIFT");
          await sleep(200);
  ...
  <body>
    <div class="container">
      ...
      <div class="hidden-note">
        ⚠️ <strong>Hidden networks</strong> are included in scans. If your
        network does not broadcast its SSID, you can also manually enter the
        SSID and BSSID.
      </div>
      <label>SSID</label>
      ...
      <button onclick="forget()" class="secondary" style="background: #722">
        Forget
      </button>
      <br />
      <button onclick="window.location.href = '/'" style="background: #333">
        ← Back to HID
      </button>
    </div>
    <script>
        ...
        fetch("/sta/status", { cache: "no-store" })
          ...
          .then((data) => {
            ...
            if (data.connected) {
              ...
              box.appendChild(
                document.createTextNode("IP: " + (data.ip || "")),
              );
            } else {
              const status = document.createElement("span");
              ...
              box.appendChild(status);
              box.appendChild(
                document.createTextNode(" – " + (data.error || "Idle")),
              );
            }
          })
          .catch(() => {});
        ...
        fetch("/sta/scan", { cache: "no-store" })
          ...
          .then((data) => {
            if (data && data.scanning) {
              listDiv.innerHTML =
                '<div class="scanning-msg">Scanning, please wait...</div>';
              scanAttempts++;
              if (scanAttempts < MAX_SCAN_ATTEMPTS)
                setTimeout(scanNetworks, 2000);
              else {
                listDiv.innerHTML =
                  '<div class="scanning-msg error-msg">Scan timed out.</div>';
                scanAttempts = 0;
              }
              return;
            }
            ...
            if (data.length === 0) {
              listDiv.innerHTML =
                '<div class="scanning-msg">No networks found.</div>';
              scanAttempts = 0;
              return;
            }
            listDiv.innerHTML = "";
            data.forEach((net) => {
              ...
              item.addEventListener("click", () => {
                selectNetwork(net.ssid || "", net.bssid || "");
              });
          ...
          .catch((err) => {
            listDiv.innerHTML =
              '<div class="scanning-msg error-msg">Error scanning. Refresh to retry.</div>';
            scanAttempts = 0;
            console.error(err);
          });
      }
...
void setup() {
  ...
  Keyboard.begin();
  Mouse.begin();
  LOG_INFO("USB HID initialised, host should now see the device");
  loadSettings();
  WiFi.mode(WIFI_AP);
