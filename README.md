# ESP32-HID-Web-Remote-Controller - V3

**ESP32‑S3 USB HID (Mouse/Keyboard) with Wi‑Fi AP, captive portal, persistent settings, and STA (client) mode**  
...
- 📡 **Wi‑Fi scanning** – scan for networks and connect via the web interface.
...
- 📊 **On‑board logging** – view logs in the web UI to help debug; raw logs available at `/logs`.
- 🎛 **Adjustable Settings** – sensitivity, repeat interval, and legacy mode (slower key presses for older hosts) – settings are saved automatically.
...
- **Settings** – sensitivity and repeat interval sliders, legacy mode checkbox (slower key timing). Settings are saved automatically to flash.
- **Logs** – view client‑side logs; raw logs available at `/logs`.
...
> **Note:** The ESP32 remembers the STA credentials and will attempt to reconnect after each power‑cycle (with retries).
...
|---------|---------------------------|
| **TV/computer does not recognise USB HID** | 1. Ensure **USB CDC On Boot** is **Disabled** in board settings.<br>2. Try a different USB cable (data‑capable).<br>3. Power the ESP32 externally if the USB port can't supply enough current.<br>4. The code uses `Mouse.begin(); Keyboard.begin(); USB.begin();` – this order is known to work on most hosts. |
